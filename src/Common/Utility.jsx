...
export default class Utility {
  ...
  constructor() {
    ...
    // console.log('------------utility----------------');
    try {
      // console.log('http url instance');
      // this.HttpUrl = new HttpUrl();
      // console.log('create http url instance success');
    } catch (ex) {
   ...
   * 清除搜索框中的缓存项
   */
  static removeTempSearchKey() {
    this.removeContent(this.constItem.SaveTempSearchKey);
  }

  /**
   * 获得搜索框中的缓存项
   */
  static getTempSearchKey(title) {
    const tempSearchKey = this.getContent(this.constItem.SaveTempSearchKey);
    return this.isInvalid(tempSearchKey) || this.isInvalid(tempSearchKey[title]) ? '' : tempSearchKey[title];
  }

  /**
   * 保存搜索框中的缓存项
   */
  static saveTempSearchKey(title, searchKey) {
    const tempSearchKey = this.isInvalid(this.getContent(this.constItem.SaveTempSearchKey)) ? {} : this.getContent(this.constItem.SaveTempSearchKey);
    tempSearchKey[title] = searchKey;
    this.setContent(this.constItem.SaveTempSearchKey, tempSearchKey);
  }
